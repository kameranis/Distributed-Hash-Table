...
class Server(object):
    ...
    def _depart(self,data,sock):
        answer  = 'depart:' + self.neighbors.get_back() + ':' + self.neighbors.get_front() + ':' + str(self.PORT)
    ...
    def _insert(self, data, sock):
        ...
        if self.belongs_here(key):
            ...
            #self.message_queues[sock].put(self.neighbors.send_front('add:' + ':'.join(x[-3:])))
            threading.Thread(target=send_request, args=(self.neighbors.front_port,'add:' + ':'.join(x[-3:],))).start()
        else:
